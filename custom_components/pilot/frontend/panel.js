...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var PilotPanel = class extends i4 {
    ...
    connectedCallback() {
      ...
      void this._refresh();
      this._timer = window.setInterval(() => void this._refresh(), 3e4);
    }
    disconnectedCallback() {
    ...
    render() {
      if (this._loading) return b2`<p class="empty">Loading…</p>`;
      const s4 = this._status;
      const fresh = s4.last_update_success !== false && (s4.vitrine_age_s == null || s4.vitrine_age_s <= 60);
      return b2`
      <h1>Pilot</h1>

      <div class="card row">
        <div class="metric">
          <div class="value ${fresh ? "ok" : "bad"}">${s4.status ?? "?"}</div>
          <div class="label">runtime ${s4.runtime_version ?? ""}</div>
        </div>
        <div class="metric">
          <div class="value">${s4.cost_today ?? "\u2014"}</div>
          <div class="label">cost today (₽)</div>
        </div>
        <div class="metric">
          <div class="value">${s4.queue_size ?? 0}</div>
      ...
      ${this._queue.length === 0 ? b2`<div class="card empty">Nothing awaits your decision.</div>` : this._queue.map(
        (item) => b2`
              <div class="card">
                <div><strong>${item.title}</strong></div>
                ${item.summary ? b2`<div class="empty">${item.summary}</div>` : A}
      ...
      <div class="card">
        ${this._vitrine.error ? b2`<span class="bad">Vitrine unavailable — ${this._vitrine.error}</span>` : b2`<pre>${(this._vitrine.lines ?? []).join("\n")}</pre>`}
      </div>
    `;
    }
  };
